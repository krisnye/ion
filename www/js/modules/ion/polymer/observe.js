(function(){require.register('ion/polymer/observe',function(module,exports,require){// Copyright 2012 Google Inc.
//
...
(function(global) {
  ...
  Path.prototype = createObject({
    ...
    getValueFrom : function _getValueFrom(obj, observedSet) {
    ...
    compiledGetValueFromFn : function _compiledGetValueFromFn() {
    ...
    setValueFrom : function _setValueFrom(obj, value) {
      if (!this.length)
        return false;
  ...
  invalidPath.getValueFrom = invalidPath.setValueFrom = function _setValueFrom() {};
  ...
  function Observer(object, callback, target, token) {
    ...
    if (hasObserve) {
      var self = this;
      this.boundInternalCallback = function _boundInternalCallback(records) {
  ...
  Observer.prototype = {
    internalCallback : function _internalCallback(records) {
      if (this.closed)
        return;
    ...
    deliver : function _deliver(testingResults) {
      if (this.closed)
        return;
      if (hasObserve) {
    ...
    invokeCallback : function _invokeCallback(args) {
      try {
  ...
  global.Platform.performMicrotaskCheckpoint = function _performMicrotaskCheckpoint() {
  ...
  if (collectObservers) {
    global.Platform.clearObservers = function _clearObservers() {
      allObservers = [];
    };
  }
  ...
  ObjectObserver.prototype = createObject({
    ...
    sync : function _sync(hard) {
      if (!hasObserve)
        this.oldObject = copyObject(this.object);
    },

    check : function _check(changeRecords) {
      var diff;
      var oldValues;
      if (hasObserve) {
        if (!changeRecords)
  ...
  ArrayObserver.applySplices = function _applySplices(previous, current, splices) {
  ...
  PathObserver.prototype = createObject({
    ...
    setValue : function _setValue(newValue) {
      if (!this.path)
        return;
  ...
  CompoundPathObserver.prototype = createObject({
    ...
    addPath : function _addPath(object, path) {
      if (this.started)
  ...
  PathObserver.defineProperty = function _defineProperty(object, name, descriptor) {
  ...
  ArraySplice.prototype = {
    ...
    calcEditDistances : function _calcEditDistances(current, currentStart, currentEnd,
                                old, oldStart, oldEnd) {
      // "Deletion" columns
    ...
    // matrix.
    spliceOperationsFromEditDistances : function _spliceOperationsFromEditDistances(distances) {
     ...
     */
    calcSplices : function _calcSplices(current, currentStart, currentEnd,
                          old, oldStart, oldEnd) {
      var prefixCount = 0;
      var suffixCount = 0;
    ...
    sharedPrefix : function _sharedPrefix(current, old, searchLength) {
    ...
    sharedSuffix : function _sharedSuffix(current, old, searchLength) {
    ...
    calculateSplices : function _calculateSplices(current, previous) {
      return this.calcSplices(current, 0, current.length, previous, 0,
                              previous.length);
    },

    equals : function _equals(currentValue, previousValue) {
  ...
  global.ArrayObserver.calculateSplices = function _calculateSplices(current, previous) {
