...
export default class Compiler {
    ...
    getExternalReference(path, sourceNode?) {
        if (this.isValidExternalReference(path, null)) {
            return new ast.ExternalReference(sourceNode, {moduleName: path })
        }
        ...
        if (this.isValidExternalReference(moduleName, exportName)) {
            return new ast.ExternalReference(sourceNode, { moduleName, exportName })
        }
        return null
    }
    ...
    getModule(name: string, required: boolean = false): ModuleCompiler | null {
        ...
        if (!this.modules.has(name)) {
            ...
            if (filename != null) {
                ...
                module = this.modules[name] = new ModuleCompiler(this, name, filename, source)
            }
