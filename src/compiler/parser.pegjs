...
BitwiseOrExpression = start:start head:BitwiseXorExpression tail:( _ ("|") _ BitwiseXorExpression end)* { return leftAssociateBinaryExpressions(start, head, tail) }
...
decimal = [+-]? (decimalInteger ("." decimalDigit+)? / "." decimalDigit+) ([eE] [+-]? decimalDigit+)? !identifierPart { return parseFloat(text()) }
integer = [+-]? decimalInteger !identifierPart { return parseInt(text()) }
decimalDigit = [0-9]
nonzeroDigit = !"0" decimalDigit
decimalInteger = "0" / nonzeroDigit decimalDigit*
hexInteger = "0x" hexDigit+ { return parseInt(text(), 16) }
hexDigit = [0-9a-fA-F]
...
null = "null" !identifierPart { return null }
undefined = "undefined" !identifierPart { return undefined }
and = "and" !identifierPart { return "&&" }
or = "or" !identifierPart { return "||" }
