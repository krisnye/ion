        ...
        properties: {
            ...
            createRuntime: {
                ...
                value: function (context, ast) {
                    ...
                    return ion.patch.combine(new type(properties), { ast: ast });
                }
            },
            canCache: {
                writable: true,
...
var lookup = {
        type: {
            UndoStatement: ion.patch.combine(new Factory(), { runtime: './UndoStatement' }),
            VariableDeclaration: ion.patch.combine(new Factory(), { runtime: './VariableDeclaration' }),
            ThisExpression: ion.patch.combine(new Factory(), {
            ...
            Identifier: ion.patch.combine(new Factory(), {
                createRuntime: function (context, ast) {
                    var expression = context.getVariable(ast);
                    if (ast.deep) {
                        expression.deep = true;
                    }
                    return expression;
            ...
            Function: ion.patch.combine(new Factory(), {
                createRuntime: function (context, ast) {
                    var value = ast.value;
                    if (ast.context) {
                        value = value(context);
            ...
            Template: ion.patch.combine(new Factory(), { runtime: './Template' }),
            Literal: ion.patch.combine(new Factory(), {
                runtime: './Literal',
                canCache: function (ast) {
                    return true;
                },
            ...
            Property: ion.patch.combine(new Factory(), { runtime: './Property' }),
            IfStatement: ion.patch.combine(new Factory(), { runtime: './IfStatement' }),
            BlockStatement: ion.patch.combine(new Factory(), { runtime: './BlockStatement' }),
            ReturnStatement: ion.patch.combine(new Factory(), { runtime: './ReturnStatement' }),
            ObjectExpression: ion.patch.combine(new Factory(), { runtime: './ObjectExpression' }),
            ArrayExpression: ion.patch.combine(new Factory(), { runtime: './ArrayExpression' }),
            ExpressionStatement: ion.patch.combine(new Factory(), { runtime: './ExpressionStatement' }),
            ForOfStatement: ion.patch.combine(new Factory(), { runtime: './ForInOfStatement' }),
            ForInStatement: ion.patch.combine(new Factory(), { runtime: './ForInOfStatement' }),
            MemberExpression: ion.patch.combine(new Factory(), {
            ...
            CallExpression: ion.patch.combine(new Factory(), { runtime: './CallExpression' }),
            NewExpression: ion.patch.combine(new Factory(), { runtime: './CallExpression' }),
            UnaryExpression: {
                operator: {
                    '!': ion.patch.combine(new Factory(), {
                        evaluate: function (a) {
                            return !a;
                        }
                    }),
                    'typeof': ion.patch.combine(new Factory(), {
                        evaluate: function (a) {
                            return typeof a;
                        }
                    }),
                    'void': ion.patch.combine(new Factory(), {
                        evaluate: function (a) {
                            return void a;
                        }
                    }),
            ...
            ConditionalExpression: ion.patch.combine(new Factory(), { runtime: './ConditionalExpression' }),
            BinaryExpression: {
                operator: {
                    '*': ion.patch.combine(new Factory(), {
