void (function(){var _ion_compiler_postprocessor_ = function(module,exports,require){var activateStatements, addOrderPropertyToStatements, addPropertyDeclaration, addStatement, addUseStrictAndRequireIon, arrayComprehensionsToES5, assertStatements, basicTraverse, block, callFunctionBindForFatArrows, checkVariableDeclarations, classExpressions, convertForInToForLength, convertNodeToLiteralNode, createForInLoopValueVariable, createTemplateFunctionClone, createTemplateRuntime, defaultAssignmentsToDefaultOperators, defaultOperatorsToConditionals, destructuringAssignments, ensureIonVariable, existentialExpression, extractForLoopRightVariable, extractForLoopsInnerAndTest, extractReactiveForPatterns, falseExpression, forEachDestructuringAssignment, functionDeclarations, functionParameterDefaultValuesToES5, getExternalIdentifiers, getPathExpression, getReferenceIdentifiers, hoistVariables, ion, ionExpression, isAncestorObjectExpression, isConstantLiteral, isFunctionNode, isPattern, isReferenceNode, isSimpleObjectExpression, isSuperExpression, isVariableReference, javascriptExpressions, letAndConstToVar, namedFunctionsAndNewArguments, nodeToLiteral, nodejsModules, nodes, nullExpression, patchAssignmentExpression, propertyDefinitions, propertyStatements, removeLocationInfo, setNodeOutputValues, spreadExpressions, superExpressions, thisExpression, traverse, trueExpression, typedObjectExpressions, undefinedExpression, validateTemplateNodes, variableDeclarationExpressions, wrapTemplateInnerFunctions, _ref;
...
isConstantLiteral = function(node, parent, key) {
  var computed, index, value, _i, _j, _len, _len1, _ref1, _ref2, _ref3;
  if (node == null) {
    return false;
  }
  ...
  if (node.type === 'ObjectExpression') {
    ...
    for (index = _i = 0, _len = _ref1.length; _i < _len; index = ++_i) {
      _ref2 = _ref1[index], value = _ref2.value, computed = _ref2.computed;
      if (computed || !isConstantLiteral(value, node, index)) {
        return false;
      }
    }
    return true;
...
convertNodeToLiteralNode = function(node) {
  if (node.type === 'Literal') {
    return node;
  }
  return {
...
nodeToLiteral = function(object) {
  ...
  } else if ((object != null) && (object.type === 'ArrayExpression' || object.type === 'ObjectExpression') && isConstantLiteral(object)) {
    node = {
